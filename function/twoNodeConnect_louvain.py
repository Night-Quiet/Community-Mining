...
def twoConnect_louvain(g, describe="Karate_club", type_set_1=0, type_set_2=0):
    """
        :param g: 图G
        :param describe: 二点连通社区划分结果，默认为Karate_club
        :param type_set_1: 为0：最大社区使用louvain算法划分，为1：最大社区使用LPA算法划分，默认为0
        :param type_set_2: 为0：最后聚点划分使用louvain算法，为1：最后聚点划分使用LPA算法，默认为0
    ...
    ****************如需了解算法原理****************
    ...
    # 获取地址
    # str_location_comm：二点连通结果地址转换
    # str_location_node：二点连通割点结果地址转换
    ...
    Connect_result = read_csv(str_location_comm)                                      # 社区
    ...
    if type_set_1 == 0:
        lv_temp = community_louvain.best_partition(g_subgraph)
        dict_out = {}
        for key, value in lv_temp.items():
            dict_out.setdefault(value, []).append(key)
        lv_bigOne = [list(v) for v in dict_out.values()]
    else:
        lpa_temp = LPA(g_subgraph)
        lv_bigOne = [list(val) for val in lpa_temp]
    BigOne_result = copy.deepcopy(lv_bigOne)
    ...
    for cut_node in Cut_node:
        cutNode_comm = node_commNum[cut_node[0]]  # 以空手道为例：割点0所属社区有0 3 4
        for commNum_1 in range(len(cutNode_comm)):
            for commNum_2 in range(commNum_1 + 1, len(cutNode_comm)):
                weight_1 = commNum_neighborCount[cutNode_comm[commNum_1]][cut_node[0]]  # 第一个循环值为 社区0中割点0的权重
                weight_2 = commNum_neighborCount[cutNode_comm[commNum_2]][cut_node[0]]  # 第一个循环值为 社区3中割点0的权重
                edge_add.append(
                    ("C" + str(cutNode_comm[commNum_1]), "C" + str(cutNode_comm[commNum_2]), weight_1 + weight_2))
        Cut_node_comm[cut_node[0]] = cutNode_comm
        for comm in cutNode_comm:
            Cut_node_trans["C" + str(comm)].append(cut_node[0])
    ...
    # lv：louvain算法社区划分结果
    if type_set_2 == 0:
        lv_temp = community_louvain.best_partition(g_new)
        dict_out = {}
        for key, value in lv_temp.items():
            dict_out.setdefault(value, []).append(key)
        lv = [list(v) for v in dict_out.values()]
    else:
        lpa_temp = LPA(g_new, 'weight')
        lv = [list(val) for val in lpa_temp]
    ...
    print_csv("./save/connect/" + describe + "/" + "node.csv", Cut_node)                  # 割点储存
    ...
    print_csv("./save/connect/" + describe + "/" + "lv_gather.csv", lv)                   # 聚点社区划分储存
    print_csv("./save/connect/" + describe + "/" + "endResult.csv", lv_reduce)            # 聚点还原最终社区划分储存
    dump_object(g_new, "../save/connect/" + describe + "/" + "gNew.graph")                 # 聚点图储存
